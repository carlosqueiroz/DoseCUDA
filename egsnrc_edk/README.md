...
- Configs YAML: grade padrão de energias 10X (0.2–20 MeV), 48 cones de 3,75°, `dr=0.5 mm`, `rmax=20 cm`.
...
- **Raio:** `dr = 0.5 mm` (opção 1.0 mm), `rmax = 20 cm`. `r_edges` armazenados incluem 0; os raios de EDKnrc usam as bordas externas `r_edges[1:] = [dr, 2dr, ..., rmax]`.
...
1. `generate_edknrc_inputs.py` → um input por energia (`ANGLES` usa um único Δθ, `CAVITY ZONES=0`, raios `dr..rmax`).
...
3. `parse_edknrc_output.py` lê `.egsdat` (somas de energia por região), divide por `N_hist` e por `E_inc` → `Kdiff` já normalizado; constrói `Kcum`, valida, salva `.npz/.npy` e CSV debug opcional.
